...
import "./DepositAndWithdrawals.sol"; //Import DepositAndWithdrawals

contract Main {
    InterestModule public interestModule; // Instance of InterestModule
    DepositAndWithdraw public depoistAndWithdrawals; //depoistAndWithdrawals
    ...
    constructor(address payable _interestModuleAddress,address payable _depoistAndWithdrawalsAddress) {
        owner = msg.sender;
        paused = false;
        interestModule = InterestModule(_interestModuleAddress); // Initialize the InterestModule
        depoistAndWithdrawals = DepositAndWithdraw(_depoistAndWithdrawalsAddress); 
    }
    ...
    function addFunds() external greaterZero payable {
        ...
        //WenKang Code
        interestModule.updateLastInterestCalculationTime(msg.sender);
        //End
    }
    ...
    function checkTimeRemaining(address _child) external view returns (uint256) {
        return depoistAndWithdrawals.checkTimeRemaining(block.timestamp,accounts[_child].contractEndTime);
    }
...
// Function to copy User details into Account
    ...
    function withdrawInterest() public  {
        ...
        if(interestModule.withdrawInterest(msg.sender,accounts[msg.sender].balance)){
            payable(msg.sender).transfer(interestModule.calculateInterest(msg.sender,accounts[msg.sender].balance));
        }  
    }

    function addInterest() public  {
        accounts[msg.sender].balance += interestModule.addInterest(msg.sender,accounts[msg.sender].balance);
    }
    ...
    function distributeInterest(uint256 percentage, address toAccount) public  {
        if(interestModule.distributeInterest(percentage, toAccount, msg.sender,accounts[msg.sender].balance)){
            payable(toAccount).transfer(interestModule.calculateInterest(msg.sender,accounts[msg.sender].balance) * percentage / 100);
            payable(msg.sender).transfer(interestModule.calculateInterest(msg.sender,accounts[msg.sender].balance) * (100 - percentage) / 100);
        }
    ...
    function getLastInterestCalculationTime() public view returns (uint256) {
        return interestModule.getLastInterestCalculationTime(msg.sender);
    }
